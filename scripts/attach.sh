...
VM03_SID_DEV="${VM03_SID_DEV:-enp3s0}"
VM04_SID_DEV="${VM04_SID_DEV:-enp2s0}"
...
ssh -F "${INFRA_DIR}/ssh_config" vm03 \
    "sudo ip -6 route replace 'fd00:a:3:0:8200::/80' encap bpf xmit obj '$REMOTE_OBJ' section lwt_xmit/tlv_embedder dev '$VM03_SID_DEV' && \
     ip -6 route show 'fd00:a:3:0:8200::/80'"
ssh -F "${INFRA_DIR}/ssh_config" vm04 \
    "sudo ip -6 route replace 'fd00:a:4:0:8300::/80' encap bpf xmit obj '$REMOTE_OBJ' section lwt_xmit/tlv_selector dev '$VM04_SID_DEV' && \
     ip -6 route show 'fd00:a:4:0:8300::/80'"
