...
SEGS="fd00:a:3:0:8200:1::,fd00:a:4:0:8300::,fd00:a:7::1,fd00:a:5::d4"
...
ssh -F "$SSH_CONFIG" vm02 \
    "chmod +x '$REMOTE_ENCAP' && sudo '$REMOTE_ENCAP' add --prefix 10.5.0.0/24 --dev enp3s0 --tlv-type 252 --tlv-len 1 --segs '$SEGS'"

if [[ -n "${PING_INTERVAL:-}" ]]; then
    ssh -F "$SSH_CONFIG" vm01 "sudo ping -c '${COUNT:-1}' -i '$PING_INTERVAL' 10.5.0.6"
else
    ssh -F "$SSH_CONFIG" vm01 "ping -c '${COUNT:-1}' 10.5.0.6"
fi
